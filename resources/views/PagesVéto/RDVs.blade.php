    ...
    <table class="table">
        ...
        <tbody>
            @foreach($rendezVs as $rendezV)
                ...
                <tr>
                    <td colspan="6">
                        <form action="{{ route('RDVs.approve', $rendezV->id) }}" method="POST" style="display: inline-block;">
                            @csrf
                            <button type="submit" class="btn btn-success">Approuver</button>
                        </form>
                        <form action="{{ route('RDVs.reject', $rendezV->id) }}" method="POST" style="display: inline-block;">
                            @csrf
